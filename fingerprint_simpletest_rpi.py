...
def get_fingerprint():
    ...
    screen.clear()
    screen.text('Reading fingerprint:',2)
    time.sleep(1)
...
def enroll_finger(location):
    ...
    for fingerimg in range(1, 3):
        if fingerimg == 1:
            ...
            screen.clear()
            screen.text('Place your finger on the scanner:',2)
            time.sleep(1)
        else:
            print("Place same finger again...", end="")
            screen.clear()
            screen.text('Place the same finger again:',2)
            time.sleep(1)
        ...
        while True:
            ...
            if i == adafruit_fingerprint.OK:
                print("Image taken")
                screen.clear()
                screen.text('Fingerprint Captured!',2)
                break
            if i == adafruit_fingerprint.NOFINGER:
                print(".", end="")
                screen.clear()
                screen.text('Detecting',2)
            elif i == adafruit_fingerprint.IMAGEFAIL:
                print("Imaging error")
                screen.clear()
                screen.text('Fingerprint could not be read',2)
                return False
            else:
                print("Other error")
                return False
    ...
    if i == adafruit_fingerprint.OK:
        print("Created")
        screen.clear()
        screen.text('Finger Recorded!',2)
        time.sleep(1)
    else:
        if i == adafruit_fingerprint.ENROLLMISMATCH:
            print("Prints did not match")
            screen.clear()
            screen.text('Finger Prints did not match',2)
        else:
            print("Other error")
        return False
